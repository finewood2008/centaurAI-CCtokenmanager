# TOKEN MANAGER 代理功能使用指南

## 功能介绍

TOKEN MANAGER 的代理功能是一个本地 HTTP 代理服务器，可以统一管理 Claude Code、Codex 和 Gemini CLI 的 API 请求。主要特性包括：
...
在 TOKEN MANAGER 主界面，点击右上角的 **Proxy** 按钮，可以看到代理控制面板。
...
> **注意**：启用接管后，TOKEN MANAGER 会自动修改对应应用的配置文件，将 API 端点指向本地代理。原始配置会被安全备份。

### 3. 正常使用 CLI

启用接管后，你可以正常使用各个 CLI 工具。所有请求都会经过 TOKEN MANAGER 代理转发到配置的供应商。

### 4. 停止代理

当你不再需要代理时，点击 **停止代理** 按钮。TOKEN MANAGER 会：
...
TOKEN MANAGER 通过检测配置备份来判断接管状态：

- 存在备份 = 已接管
- 无备份 = 未接管

这确保了即使 TOKEN MANAGER 异常退出，重新启动后也能正确识别状态。
...
A: 点击 **停止代理** 按钮，TOKEN MANAGER 会自动恢复所有应用的原始配置。

如果 TOKEN MANAGER 异常退出，重新启动后会检测到之前的备份，你可以：
- 点击停止代理来恢复配置
- 或继续使用代理功能
...
团队网关、OIDC、SQLCipher 与管理员全文检索的部署方式见[多用户对话归档部署指南](./conversation-archive-zh.md)。`/team` 始终要求 OIDC JWT；归档及其自动恢复快照默认仅存本机，普通 S3/WebDAV 自动同步不会上传归档。

### 配置文件位置

启用接管后，TOKEN MANAGER 会修改以下配置文件：
...
原始配置备份在 TOKEN MANAGER 数据库中，停止代理时自动恢复。
