...
TOKEN MANAGER supports the `ccswitch://` deep link protocol, enabling one-click configuration import via links.
...
TOKEN MANAGER provides an online deep link generator tool:
...
Using TOKEN MANAGER's official online deep link generator tool is more convenient.
...
1. The system asks whether to open TOKEN MANAGER
2. After confirming, TOKEN MANAGER opens
...
TOKEN MANAGER automatically registers the `ccswitch://` protocol during installation.
...
```bash
/usr/bin/open -a "TOKEN MANAGER" --args --register-protocol
```
...
Before import, TOKEN MANAGER will:
