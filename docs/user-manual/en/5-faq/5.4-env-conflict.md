...
TOKEN MANAGER automatically detects conflicts between system environment variables and app configurations, preventing configurations from being unexpectedly overridden.
...
Found X environment variables that may conflict with TOKEN MANAGER configuration
[Expand] [Dismiss]
```
...
3. TOKEN MANAGER will automatically back up and remove the selected variables
...
If you prefer not to use TOKEN MANAGER to remove variables, you can handle them manually:
...
- TOKEN MANAGER provider configurations being overridden
- API requests being sent to the wrong endpoint
- Using the wrong API key

## Best Practices

1. **Use TOKEN MANAGER to manage configurations**: Avoid setting API keys in system environment variables
