...
Hermes uses YAML configuration. TOKEN MANAGER writes MCP servers to `mcp_servers`, writes editable provider entries to `custom_providers`, reads read-only entries from Hermes' `providers` dict, and updates `model.provider` / `model.default` when switching providers.
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to TOKEN MANAGER's "providers") |
...
TOKEN MANAGER's priority when modifying configurations:

1. **TOKEN MANAGER Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by TOKEN MANAGER)
- TOKEN MANAGER's `settings.json`
...
TOKEN MANAGER v3.7.0 migrated from JSON files to SQLite:
