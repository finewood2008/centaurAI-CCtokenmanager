...
This section describes how to configure TOKEN MANAGER according to your preferences.
...
TOKEN MANAGER supports four languages:
...
When enabled, TOKEN MANAGER automatically runs when the system starts.
...
Starting from v3.13.0, TOKEN MANAGER adds **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
**How to exit**: Click **Open Main Window** from the tray menu, or trigger TOKEN MANAGER via deep link / relaunch. The window is **rebuilt on demand**, with state preserved.
...
> **Use case**: If TOKEN MANAGER runs in the background for long periods and you mainly switch providers via the tray menu, enabling Lightweight Mode significantly reduces memory usage.
...
When enabled, TOKEN MANAGER automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that TOKEN MANAGER uses when opening a terminal.
...
The storage location for TOKEN MANAGER's own data, defaulting to `~/.cc-switch/`.
...
Configure TOKEN MANAGER's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, TOKEN MANAGER automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
- TOKEN MANAGER automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, TOKEN MANAGER displays details about the remote snapshot:
- Protocol version (v2)
...
Displays the current TOKEN MANAGER version number, with support for:
