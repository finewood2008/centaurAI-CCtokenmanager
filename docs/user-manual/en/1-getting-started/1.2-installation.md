...
Only download TOKEN MANAGER from **[ccswitch.io](https://ccswitch.io)**, **[GitHub Releases](https://github.com/farion1231/cc-switch/releases)**, or the project source repository. Any "TOKEN MANAGER" site or client that asks for payment, top-ups, or login credentials is not official.
...
The CLI tools managed by TOKEN MANAGER (Claude Code, Codex, Gemini CLI) require a Node.js environment.
...
2. Open the DMG, or extract the zip to get `TOKEN MANAGER.app`
3. Drag it to the Applications folder

### Signed and Notarized

TOKEN MANAGER for macOS is signed and notarized by Apple. You can install and open it directly — no extra steps needed.
...
After installation, launch TOKEN MANAGER:

1. The app window displays correctly
2. A TOKEN MANAGER icon appears in the system tray
3. The app switcher shows enabled managed apps, and you can switch to the target app panel

## Auto Update

TOKEN MANAGER includes built-in auto-update functionality:
...
- Move `TOKEN MANAGER.app` to Trash
