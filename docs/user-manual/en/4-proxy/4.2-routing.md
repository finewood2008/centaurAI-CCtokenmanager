...
App routing means letting TOKEN MANAGER route a specific application's API requests through the local routing service.
...
When routing is enabled, TOKEN MANAGER modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, TOKEN MANAGER will:
...
Before enabling routing, TOKEN MANAGER backs up the original configuration and restores it when disabled.
