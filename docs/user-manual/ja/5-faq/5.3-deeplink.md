...
TOKEN MANAGER は `ccswitch://` ディープリンクプロトコルをサポートしており、リンクからワンクリックで設定をインポートできます。
...
TOKEN MANAGER はオンラインのディープリンク生成ツールを提供しています：
...
TOKEN MANAGER 公式のオンラインディープリンク生成ツールを使用するとより便利です。
...
1. システムが TOKEN MANAGER を開くかどうかを確認
2. 確認後、TOKEN MANAGER が起動
3. インポート確認ダイアログを表示
4. インポートを確認
...
TOKEN MANAGER のインストール時に `ccswitch://` プロトコルが自動登録されます。
...
インポート前に TOKEN MANAGER は以下を実行します：
