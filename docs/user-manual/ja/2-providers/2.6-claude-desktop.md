...
Claude Desktop パネルでは、TOKEN MANAGER 上で Claude Desktop のプロバイダー設定を管理できます。
...
Claude Desktop と Claude Code は別々のアプリ入口です。Claude Code は `~/.claude/settings.json` を使い、Claude Desktop は専用の 3P profile 設定を使います。TOKEN MANAGER 上でも「Claude」と「Claude Desktop」として別々に表示され、アイコン右下の小さなバッジで区別できます。
...
| サードパーティモード | TOKEN MANAGER が管理する 3P profile に書き込み |
| MCP / Skills | Claude Desktop 3P profile は TOKEN MANAGER の MCP / Skills 同期を使いません |
...
多くのユーザーはまず Claude Code 側でプロバイダーを設定し、その同じプロバイダー群を Claude Desktop に持ち込みたいはずです。TOKEN MANAGER の初回起動時、または Claude Desktop パネルを初めて開いたときにプロバイダーがまだない場合は、**Claude Code の既存プロバイダーをインポート** をクリックします。
...
- ルーティングが必要なプロバイダー：TOKEN MANAGER を起動したまま、Claude Desktop ローカルルーティングをオンにし、Claude Desktop を再起動します
...
直結モードでは、TOKEN MANAGER が Claude Desktop の 3P profile をプロバイダーのエンドポイントへ向けます。
...
- 形式変換が不要
- 使用中に TOKEN MANAGER のローカルルーティングを起動し続ける必要がない
...
モデルマッピングモードは、プロバイダーのモデルが `claude-sonnet-*` / `claude-opus-*` / `claude-haiku-*` の三つの役割 ID でない場合（旧式 Claude ID や DeepSeek、Kimi などの非 Claude モデルを含む）、または TOKEN MANAGER による API 形式変換が必要な場合に適しています。

**モデルマッピングが必要** をオンにすると、Claude Desktop は TOKEN MANAGER のローカルゲートウェイへ接続します。

```text
http://127.0.0.1:15721/claude-desktop
```

TOKEN MANAGER は次を担当します。
...
4. TOKEN MANAGER に保存されたプロバイダー認証情報でアップストリームへアクセスする
...
理由は、現在の Claude Desktop が Sonnet / Opus / Haiku の役割ファミリー以外のモデルを拒否するためです。そのため TOKEN MANAGER のルーティング機能で一度モデルマッピングを行います。
...
モデルマッピングモードでは、リクエスト変換のために TOKEN MANAGER ローカルルーティングが必要です。ローカルルーティングは強力ですが少し複雑な機能でもあるため、ルーティングを必要としないユーザーの誤操作を避けるために、メインページのルーティング切り替えはデフォルトで非表示です。必要なときだけ手動で表示してください。
...
TOKEN MANAGER は Claude Desktop の公式 1P モードを復元し、TOKEN MANAGER が管理する 3P profile を削除します。

公式モードでは API Key もローカルルーティングも不要です。

Claude Code からプロバイダーをインポートするとき、TOKEN MANAGER は自動的に **Claude Desktop Official** も追加します。

## 設定ファイルの場所

TOKEN MANAGER は Claude Desktop の 3P 設定ディレクトリに書き込みます。
...
これらのファイルは TOKEN MANAGER が自動管理します。手動編集はおすすめしません。設定の不整合が起きた場合は、現在のプロバイダーを再度有効化すると通常は修復できます。
...
| ローカルルーティング token が未生成 | そのプロバイダーへ再度切り替えると、TOKEN MANAGER が新しい token を書き込みます |
...
### TOKEN MANAGER を閉じてもよい？

モードによります。

- 直結モード：Claude Desktop が再起動して設定を読み込んだ後は、ローカルルーティングを起動し続ける必要はありません
- モデルマッピングモード：TOKEN MANAGER を起動したままにし、Claude Desktop ローカルルーティングもオンにしておく必要があります

### 実際のアップストリームモデル名は Claude Desktop に書き込まれる？

モデルマッピングモードでは書き込まれません。Claude Desktop profile には安全な Sonnet / Opus / Haiku 役割ルートと表示名だけが保存されます。実際のアップストリームモデル名は TOKEN MANAGER のプロバイダー設定に保存され、ローカルゲートウェイを通るリクエスト時にマッピングされます。
