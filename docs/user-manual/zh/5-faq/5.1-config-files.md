...
├── conversation-archive.db # 对话归档 SQLCipher 数据库（启用后）
├── settings.json     # 设备级设置
├── secrets/          # 托管归档密钥（使用环境变量时不会创建）
├── archive-backups/  # 默认的本地归档恢复快照目录
├── skills/           # 技能 SSOT 目录
...
`conversation-archive.db` 与 `archive-backups/` 默认只保存在本机，普通 S3/WebDAV 自动同步不会包含它们。本地归档快照默认最短间隔 15 分钟、保留 30 份，目录可以在归档设置中改为其他本机绝对路径。

本地快照默认包含恢复密钥；Unix 上快照目录权限为 `0700`、文件权限为 `0600`。取得快照目录可能意味着可以解密归档，应把整个目录作为敏感数据保护。手动远端归档操作属于逐次明确选择，默认关闭，且远端快照不包含解密密钥。
...
Hermes 使用 YAML 配置。TOKEN MANAGER 将 MCP 服务器写入 `mcp_servers`，将可编辑的供应商条目写入 `custom_providers`，读取 Hermes `providers` 字典中的只读条目，并在切换供应商时更新 `model.provider` / `model.default`。
...
| 字段 | 说明 |
|------|------|
| `models.providers` | 供应商配置（映射为 TOKEN MANAGER 的"供应商"） |
| `env` | 环境变量配置 |
...
TOKEN MANAGER 修改配置时的优先级：

1. **TOKEN MANAGER 数据库** - 单一事实源 (SSOT)
...
- CLI 工具的配置文件（会被 TOKEN MANAGER 回填）
- TOKEN MANAGER 的 `settings.json`
...
TOKEN MANAGER v3.7.0 从 JSON 文件迁移到 SQLite：
