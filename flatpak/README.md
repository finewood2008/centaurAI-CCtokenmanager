...
This directory contains the Flatpak manifest (`com.centaurai.tokenmanager`) for TOKEN MANAGER, used to convert the generated `.deb` artifact into an installable `.flatpak` package via CI or local builds.
...
```bash
flatpak-builder --force-clean --user --disable-cache --repo flatpak-repo flatpak-build flatpak/com.centaurai.tokenmanager.yml
flatpak build-bundle --runtime-repo=https://flathub.org/repo/flathub.flatpakrepo flatpak-repo TOKEN-MANAGER-Linux.flatpak com.centaurai.tokenmanager
```

4) Install and run:

```bash
flatpak install --user ./TOKEN-MANAGER-Linux.flatpak
flatpak run com.centaurai.tokenmanager
```
...
If you prefer minimal permissions (e.g., for Flathub submission or security concerns), you can replace `--filesystem=home` in `flatpak/com.centaurai.tokenmanager.yml` with more precise grants:
...
If you plan to publish on Flathub or want stricter permission control, adjust the `finish-args` in `flatpak/com.centaurai.tokenmanager.yml` accordingly.
